...
public class ArrayBag<T> implements BagInterface<T> {
       ...
       * @return 'true' if the bags are equal and 'false' otherwise
       ...
       */
       ...
       public boolean equals(ArrayBag<T> aBag) {
             ...
             if(aBag.getCurrentSize() == getCurrentSize()) // check both bags are of same size

             {

                    result = true;

                    // loop over the bag and check if the frequency of each item is same in both the bags

                    for(int i=0;i<getCurrentSize();i++)

                    {

                           if(getFrequencyOf(bag[i]) != aBag.getFrequencyOf(bag[i])) // if frequency are not same then basg are not equal
       ...
       public boolean removeAllEntriesOf(T anEntry) {
             ...
             int index = getIndexOf(anEntry); // get the index of anEntry in bag

            

             // loop that continues till all the entries are removed

             // index = -1 ,means anEntry not found in bag

             while(index != -1)

             {

                    removeEntry(index); // remove the element at the index

                    index = getIndexOf(anEntry); // get the index of the entry
